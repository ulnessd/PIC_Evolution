...
def _dt_burstiness(dt_series: List[int]) -> float:
    if not dt_series:
        return 0.0
    m = sum(dt_series) / len(dt_series)
    if m <= 0:
        return 0.0
    v = sum((x - m) ** 2 for x in dt_series) / len(dt_series)
    return (v ** 0.5) / m


def _coordinated_change_index(a_stream: List[int], b_stream: List[int], lag: int = 1) -> float:
    """Lag-tolerant coordinated-change index on 1-bit streams."""
    if len(a_stream) < 2 or len(b_stream) < 2:
        return 0.0
    ch_a = [i for i in range(1, len(a_stream)) if a_stream[i] != a_stream[i - 1]]
    ch_b = [i for i in range(1, len(b_stream)) if b_stream[i] != b_stream[i - 1]]
    ...
    for t in ch_a:
        for d in range(-lag, lag + 1):
            if (t + d) in bset:
                hits += 1
                break
    return hits / max(1, len(ch_a))


def _opcode_entropy_norm(genome_a: List[List[int]], genome_b: List[List[int]]) -> float:
    """Shannon entropy of opcode IDs, normalized by log2(41) and clipped to <1."""
    seq = []
    for inst in (genome_a + genome_b):
        if isinstance(inst, (list, tuple)) and len(inst) >= 1:
            seq.append(int(inst[0]))
    if not seq:
        return 0.0
    counts = {}
    for x in seq:
    ...
    for c in counts.values():
        p = c / n
        H -= p * math.log(p, 2)
    return min(0.999, H / math.log(41, 2))


def _control_flow_span(genome_a: List[List[int]], genome_b: List[List[int]]) -> float:
    """Fraction of instructions that are control-flow or skip-like."""
    names = {"GOTO", "CALL", "RETURN", "RETLW", "BTFSC", "BTFSS", "DECFSZ", "INCFSZ"}
    total = 0
    flow = 0
    for inst in (genome_a + genome_b):
        if not isinstance(inst, (list, tuple)) or len(inst) < 1:
            continue
        op_id = int(inst[0])
        total += 1
        if 0 <= op_id < len(OPCODE_LIST) and OPCODE_LIST[op_id] in names:
            flow += 1
    if total == 0:
        return 0.0
    return min(0.999, flow / total)


class _TrackedPic10Sim(Pic10Sim.Pic10Sim):
    """Pic10Sim with RAM-write tracking for Phase4 MemoryUtilization metric.
    This does not change execution semantics; it only records addresses written."""
    def __init__(self):
        super().__init__()
    ...
    def write_ram(self, addr: int, value: int):
        # Mirror Pic10Sim semantics and record the actual destination.
        addr &= 0x1F
        if addr == 0x00:  # INDF -> actual address in FSR
            fsr = self.ram[0x04] & 0x1F
            super().write_ram(addr, value)
            self.ram_written.add(fsr)
            return
        super().write_ram(addr, value)
        self.ram_written.add(addr)
...
def evaluate_pair_variant3_with_reliance(
    ...
    gp2_drop_a: float = 0.0,
    gp2_drop_b: float = 0.0,
    gp2_flip_a: float = 0.0,
    gp2_flip_b: float = 0.0,
    gp2_noise_a: float = 0.0,
    gp2_noise_b: float = 0.0,
    perturb_seed: int = 0,
    ...
    def run_condition(muted: bool):
        """
        Execute one evaluation condition (live or mute) and return:
            fa, fb, a_crashed, b_crashed,
            outA2_tick, outB2_tick, outA0_tick, outB0_tick,
            dt_series,
            togA0, togB0,
            latA, latB,
            rec_list,
            memA, memB
        """
        # Deterministic per-evaluation RNG for GP2 perturbations.
        local_rng = random.Random(perturb_seed ^ (1 if muted else 0))

        def _perturb_gp2(bit: int, drop_p: float, flip_p: float, noise_p: float) -> int:
            if drop_p > 0.0 and local_rng.random() < drop_p:
                return 0
            if noise_p > 0.0 and local_rng.random() < noise_p:
                return 1 if local_rng.random() < 0.5 else 0
            if flip_p > 0.0 and local_rng.random() < flip_p:
                return bit ^ 1
            return bit

        alice = _TrackedPic10Sim()
        alice.reset()
        alice.load(alice_genome, OPCODE_LIST)

        bob = _TrackedPic10Sim()
        bob.reset()
        ...
        # Phase4 trace streams (per tick, dt-held)
        outA2_tick: List[int] = []
        outB2_tick: List[int] = []
        outA0_tick: List[int] = []
        outB0_tick: List[int] = []
        ...
        togA0 = 0
        togB0 = 0
        prevA0: Optional[int] = None
        prevB0: Optional[int] = None

        # Latency: ticks after target switch (post-grace) until first correct
        latA: List[int] = []
        latB: List[int] = []
        waitA = False
        waitB = False
        ...
        # Recovery after GP2 corruption: ticks until BOTH correct (after grace)
        rec_list: List[int] = []
        rec_wait = False
        rec_t = 0
        ...
        while t < physics_cycles:
            ...
            step_had_gp2_corruption = False

            # Step Alice
            gp2_a_raw = 0 if muted else (last_b_gp0 & 1)
            gp2_a = gp2_a_raw
            if not muted:
                gp2_a = _perturb_gp2(gp2_a, gp2_drop_a, gp2_flip_a, gp2_noise_a)
                if gp2_a != gp2_a_raw:
                    step_had_gp2_corruption = True
            ...
            # Step Bob
            gp2_b_raw = 0 if muted else (last_a_gp0 & 1)
            gp2_b = gp2_b_raw
            if not muted:
                gp2_b = _perturb_gp2(gp2_b, gp2_drop_b, gp2_flip_b, gp2_noise_b)
                if gp2_b != gp2_b_raw:
                    step_had_gp2_corruption = True
            ...
            if (not muted) and step_had_gp2_corruption:
                rec_wait = True
                rec_t = 0

            # Emit per-tick streams for dt ticks (outputs held)
            a2 = out_a & 3
            b2 = out_b & 3
            a0 = a2 & 1
            b0 = b2 & 1
            ...
            for _ in range(dt):
                outA2_tick.append(a2)
                outB2_tick.append(b2)
                outA0_tick.append(a0)
                outB0_tick.append(b0)

                if prevA0 is not None and a0 != prevA0:
                    togA0 += 1
                if prevB0 is not None and b0 != prevB0:
                    togB0 += 1
                prevA0 = a0
                prevB0 = b0
            ...
            for k in range(dt):
                ...
                # recovery window
                if rec_wait and (grace_a == 0) and (grace_b == 0):
                    if ((out_a & 1) == tgt) and ((out_b & 1) == tgt):
        ...
        if a_crashed or b_crashed:
            return 0.0, 0.0, a_crashed, b_crashed, [], [], [], [], [], 0, 0, [], [], [], set(), set()

        fa = (correct_a / valid_a) if valid_a > 0 else 0.0
        fb = (correct_b / valid_b) if valid_b > 0 else 0.0
        return fa, fb, False, False, outA2_tick, outB2_tick, outA0_tick, outB0_tick, dt_series, togA0, togB0, latA, latB, rec_list, alice.ram_written, bob.ram_written


    
    (fa_live, fb_live, a_crash, b_crash,
     outA2, outB2, outA0, outB0,
     dt_series_live,
     togA0, togB0,
     latA, latB,
     rec_list,
     memA, memB) = run_condition(muted=False)
    ...
    (fa_mute, fb_mute, a_crash_m, b_crash_m,
     _, _, _, _,
     _,
     _, _,
     _, _,
     _,
     _, _) = run_condition(muted=True)
    ...
    entropy = _entropy_ratio(outA2)

    metrics = {
        "Activity": float((togA0 + togB0) / (2.0 * max(1, physics_cycles))),
        ...
        # --- Phase4 descriptor metrics (required by Phase4_Driver_Part5 axes) ---
        "ChannelEntropyCompress": float(min(0.999, (len(zlib.compress(bytes(outA2))) / max(1, len(outA2))))),
        ...
        "ControlFlowSpan": float(_control_flow_span(alice_genome, bob_genome)),
    }

    # --- Phase4 axes helpers (behavioral; NOT fitness) ---
    def _signal_sparsity(outputs):
        # 0 = perfectly uniform usage, 1 = one-symbol dominates
        if not outputs:
            return 0.0
        counts = [0, 0, 0, 0]
        for x in outputs:
        ...
        if n == 0:
            return 0.0
        pmax = max(c / n for c in counts)
        # uniform -> pmax=0.25 => sparsity 0; single-symbol -> pmax=1 => sparsity 1
        return max(0.0, min(1.0, (pmax - 0.25) / 0.75))

    def _signal_burstiness(outputs):
        # Measures how switchy the channel is: 0 = never changes, 1 = changes every step
        ...
        for x in outputs[1:]:
            if x != prev:
                changes += 1
            prev = x
        return changes / (len(outputs) - 1)

    metrics["SignalSparsity"] = _signal_sparsity(outA2)
    metrics["SignalBurstiness"] = _signal_burstiness(outA2)
