...
BATCHES_PER_WORLD = 25_050  # How long to run each world before switching
...
def evaluate_organism(genome):
    ...
    TIMING CONTRACT (NEW):
      * The cosmos does NOT wait. Each executed instruction consumes sim.last_dt
        units of world-time (cycles), and the target/input stream advances during that time.
      * The critter's output is treated as held constant during those sim.last_dt ticks.
    ...
    # World-time index (cosmos does not wait)
    t = 0
    while t < PHYSICS_CYCLES:
        # Sample the current input bit at the instruction boundary
        bit0 = cosmos.inputs[t]
        ...
        # Clamp dt so we don't run past the end of the cosmos arrays
        if t + dt > PHYSICS_CYCLES:
            dt = PHYSICS_CYCLES - t
            if dt < 1:
                dt = 1

        # Record output once per *instruction* (keeps entropy measure comparable to old runs)
        outputs_history.append(out)

        # Toggle metric: count toggles per instruction boundary
        if last_out is None:
            last_out = out
        else:
            if out != last_out:
        ...
        # Score across the dt world-ticks that pass while output is held constant
        for k in range(dt):
            target = cosmos.targets[t + k]
    ...
    metrics = {
        # Activity is now toggles per *instruction* normalized by world horizon,
        # keeping it on a 0..~1-ish scale similar to before.
